...
export function AuthProvider({ children }) {
    ...
    const validateToken = async (token) => {
        try {
            ...
            return response.status === 200 ? response.data.data.user : null;
        } catch (error) {
            ...
            try {
                ...
                if (userData) {
                    const userInfo = {
                        ...
                        username: userData.email, // không có username thì fallback
                        is_superuser: userData.role === "ADMIN" // role xử lý thủ công
                    };
